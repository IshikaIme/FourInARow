...
_highScore highScore[5], current, tempScore;
...
void drawBall_01(int p, int q)
{
   iSetColor(255,0,0);
    ...
    //iShowBMP2(p-74,q-53,"red2.bmp", 0xFFFFFF);
}
...
void iMouse(int button, int state, int mx, int my)
{
    if(menu){
        if(button==GLUT_LEFT_BUTTON && state==GLUT_DOWN){
            ...
            //CONTINUE GAME
                ...
                fscanf(fp2,"%f",&timer);                           //Loading the TIME and TURN and MODE
                fscanf(fp2,"%d",&turn);
                fscanf(fp2,"%d",&mode);
   ...
   // else if(credit)
       // {
         //   if(mx>70&&mx<270 && my>65 && my<100)
          //      menu=1;
            //    credit=0;
        //}

    else if (play){
        if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && !gameOver && !saveFile && y[turn-1]==yStop[turn-1]){
        ...
        if(saveFile){
            if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN){
                if(mx>790 && mx<860 && my>320 && my<360){
                    ...
                    FILE* fp = fopen("Saved Game.txt","w");
...
//            FILE *fp=fopen("Get Aqua3.txt","r");
//
//                for(int i=0;i<2000;i++){
//                    fscanf(fp,"%d%d%d",&rgb[i][0],&rgb[i][1],&rgb[i][2]);
//                }
//
//            fclose(fp);
        ...
        if(key == ' ' && !gameOver && y[turn-1]==yStop[turn-1] && !saveFile && temp){
    ...
    if(mode == 1 && !gameOver){
        ...
        if(waterLevel>25 && (waterLevel-25)%100==0){
            ...
            for(int i=0;i<7;i++){
                ...
                //iShowBMP2(350,75+i*50 , "octo.bmp" , 0xFFFFFFF);
...
void updateHighScore(void)
{
    ...
    for(int i=0; i<5; i++){
        ...
        if( (current.moves < highScore[i].moves) || highScore[i].moves==0 ){
            tempScore = current;
            current = highScore[i];
            highScore[i] = tempScore;
        }

        else if( current.moves == highScore[i].moves ){
            if(current.winningTime < highScore[i].winningTime){
                tempScore = current;
                current = highScore[i];
                highScore[i] = tempScore;
            }
    ...
    FILE* fp3 = fopen("HighScore0.txt","w");

    for(int i=0;i<5;i++){
        fprintf(fp3,"%s\n",highScore[i].name);
        fprintf(fp3,"%d ",highScore[i].moves);
        fprintf(fp3,"%f\n",highScore[i].winningTime);
    }
...
int main()
{
    ...
    iSetTimer(10000, freezeBalls);
    iSetTimer(2000, Freeze);

    FILE* fp4= fopen("HighScore0.txt","r");

    for(int i=0;i<5;i++){
        fscanf(fp4,"%s",highScore[i].name);
        fscanf(fp4,"%d",&highScore[i].moves);
        fscanf(fp4,"%f",&highScore[i].winningTime);
    }
